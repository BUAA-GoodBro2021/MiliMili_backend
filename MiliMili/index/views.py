...
from video.models import Zone, UserToHistory
    ...
    if request.method == 'POST':
        ...
        if JWT == '' or JWT is None:
            result = {'result': result, 'message': message, 'not_read': -1,
                      ...
                      'view_rank_list': zone_list[0][:10],
                      'like_rank_list': zone_list[1][:10],
                      'collect_rank_list': zone_list[2][:10]}
            return JsonResponse(result)
        else:
            JWT = request.POST.get('JWT', '')
            try:
...
def index_message(request):
    if request.method == 'POST':
        ...
        result = {'result': result, 'message': message, "not_read": not_read(user_id), 'recommend_list': recommend_list,
                  'search_history_list': search_history_list, 'zone_list': zone_list[0][:10],
